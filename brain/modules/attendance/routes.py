"""
Attendance API endpoints - Using modular service
"""
...
def get_current_onsite(db: Session = Depends(get_db)):
    ...
    module = AttendanceModule(db)
    count = module.get_current_onsite()
...
def get_active_sessions(db: Session = Depends(get_db)):
    ...
    module = AttendanceModule(db)
    sessions = module.get_active_sessions()
...
def get_today_summary(db: Session = Depends(get_db)):
    ...
    module = AttendanceModule(db)
    summary = module.get_today_summary()
